...
DVRSession SessionPool::get(const std::string& rName, bool value)
{
	DVRSession s = get();
...
DVRSession SessionPool::get()
{
	Poco::Mutex::ScopedLock lock(_mutex);
    if (_shutdown) throw Poco::InvalidAccessException("Session pool has been shut down.");
	...
	if (_idleSessions.empty())
	{
		...
		{
			DVRSession newSession(DVRSessionFactory::instance().create(_connector, _connectionString));
...
void SessionPool::setFeature(const std::string& rName, bool state)
{
	...
	if (_nSessions > 0)
		throw Poco::InvalidAccessException("Features can not be set after the first session was created.");
...
bool SessionPool::getFeature(const std::string& rName)
{
	...
	if (_featureMap.end() == it)
		throw Poco::NotFoundException("Feature not found:" + rName);
...
void SessionPool::setProperty(const std::string& rName, const Poco::Any& value)
{
	...
	if (_nSessions > 0)
		throw Poco::InvalidAccessException("Properties can not be set after first session was created.");
...
Poco::Any SessionPool::getProperty(const std::string& rName)
{
	...
	if (_propertyMap.end() == it)
		throw Poco::NotFoundException("Property not found:" + rName);

	return it->second;
}


void SessionPool::applySettings(DVRSessionImpl* pImpl)
{
...
void SessionPool::customizeSession(DVRSession&)
{
}
...
void SessionPool::onJanitorTimer(Poco::Timer&)
{
	...
	{
		if ((*it)->idle() > _idleTime || !(*it)->session()->isLoggedIn())
		{	
			try	{ (*it)->session()->logout(); }
			catch (...) { }
			it = _idleSessions.erase(it);
			--_nSessions;
		}
