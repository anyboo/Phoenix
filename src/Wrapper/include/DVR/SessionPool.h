...
class DVR_API SessionPool: public Poco::RefCountedObject
...
public:
	...
	template <typename T>
	DVRSession get(const std::string& rName, const T& value)
	...
	DVRSession get(const std::string& name, bool value);
...
protected:
	virtual void customizeSession(DVRSession& session);
	...
	void applySettings(DVRSessionImpl* pImpl);
...
private:
	...
	typedef std::map<DVRSessionImpl*, PropertyPair> AddPropertyMap;
	typedef std::map<DVRSessionImpl*, FeaturePair> AddFeatureMap;
...
inline std::string SessionPool::name(const std::string& connector,
	const std::string& connectionString)
{
	return DVRSession::uri(connector, connectionString);
}
