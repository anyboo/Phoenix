...
public:
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("itemselect") ) {
			...
			if (msg.pSender->GetName() == _T("menu_Replace")) {
				if (m_pOwner) m_pOwner->GetManager()->SendNotify(m_pOwner, _T("menu_Replace"), 0, 0, true);
			}
        }
    }
    ...
    LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
    {
        ...
        CControlUI* pRoot = builder.Create(_T("xml//menu.xml"), (UINT)0, NULL, &m_pm);
